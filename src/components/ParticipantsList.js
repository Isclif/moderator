...
const ParticipantList = () => {
  return (
    <aside className="p-4 bg-gray-100 rounded-lg md:h-[502px]">
      <h2 className="text-base font-bold mb-4 text-customDark">Peoples({usersCo})</h2>
          ...
          <div key={index}  className='flex pb-1'>
            <div className="bg-gray-700 w-10 h-10 rounded-full text-white flex items-center justify-center text-sm mr-2 mb-2">image</div>
              <li className="mt-1 pl-1 flex-none w-44">
                <div className="font-bold text-xs text-customDark">{participant.name}</div>
            ...
            : 
            <div className="bg-focusedBlue w-6 h-6 rounded-full text-white flex items-center text-xs justify-center mt-3"><AudioFilled /></div>}
          </div>
            ))}
        </ul>
      </div>
